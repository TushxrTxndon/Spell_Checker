...
def build_frequency_maps(corpus_words):
    ...
    bigram_prob = defaultdict(lambda: defaultdict(lambda: 1e-9))
    for (w1, w2), c in bigram.items():
        bigram_prob[w1][w2] = c / unigram[w1]
...
def bayesian_score(word, misspelled, freq_map, dist, bigram_prob=None, prev_word=None):
    ...
    if prev_word and bigram_prob:
        P_context = bigram_prob[prev_word].get(word, 1e-9)
    else:
        P_context = 1.0
...
class SpellChecker:
    def __init__(self, wordlist=None):
        if wordlist is None:
            ...
            wordlist = [w.lower() for w in raw_words if isinstance(w, str) and w.isalpha()]
    ...
    def get_candidates(self, word, max_dist=2):
        candidates = []
        code = soundex(word)
        nearby_words = self.soundex_map.get(code, set())
        if not nearby_words:
            nearby_words = {w for w in self.dictionary if abs(len(w) - len(word)) <= max_dist}
    ...
    def correct(self, misspelled, prev_word=None, topn=3):
        ...
        if not candidates:
            return []
        scored = [
            (bayesian_score(w, misspelled, self.unigram_prob, dist,
                            bigram_prob=self.bigram_prob, prev_word=prev_word), w)
